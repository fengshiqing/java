...
public class ByUrlGetHtml {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < 1; i++) {
			...
			String htmlSourceCode = getHtmlSourceCode(url);
			...
			for (Element job : jobList) {
				String joburl = job.getElementsByClass("flexbox").getFirst().getElementsByTag("a").attr("href");
		...
		for (String str : strList) {
			String jobDesc = getHtmlSourceCode(str);
			Document document = Jsoup.parse(jobDesc);// 转换为DOM树
			Element jobcontent = document.getElementsByClass("content-word").getFirst();
	 ...
	 * @throws Exception
	 */
	private static String getHtmlSourceCode(String urlStr) throws Exception {
		...
		BufferedReader bfr = new BufferedReader(new InputStreamReader(in, StandardCharsets.UTF_8));// 建立缓冲流
		// 4.3、从缓冲流中读取数据
		StringBuilder sb = new StringBuilder();
		String line;
