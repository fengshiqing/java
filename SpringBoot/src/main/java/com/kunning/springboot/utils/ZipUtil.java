...
//            IOUtils.closeQuietly(bufferedOutStream);
            ...
            IOUtils.closeQuietly(bufferedOutStream, fileOutputStream, gzipCompressorInStream, bufferedInStream,
                    fileInStream);
            if (!file.delete()) {
    ...
    public static void unCompressTar(String tarFilePath, String outPath, long singFileSizeLimit, int numLinit) {
        ...
        try {
            ...
            // 1、关闭流
            IOUtils.closeQuietly(bufferedOutStream, fileOutputStream, tarArchiveInStream, fileInStream);
            // 2、删除解压未完成的临时文件
            if (failFlag) {
                // TODO
            }
